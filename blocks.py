...
class BasicBlockPEFT(BasicBlock):
    ...
    def __init__(
        ...
        if lora_r > 0:
            self.conv1 = ConvLoRA(nn.Conv2d, inplanes, planes, kernel_size=3, r=lora_r, stride=stride)
        else:
        ...
        if lora_r > 0:
            self.conv2 = ConvLoRA(nn.Conv2d, planes, planes, 3, lora_r)
        else:
...
class BottleneckPEFT(Bottleneck):
    ...
    def __init__(
        ...
        if lora_r > 0:
            self.conv2 = ConvLoRA(nn.Conv2d, width, width, kernel_size=3, r=lora_r, stride=stride, groups=groups, dilation=dilation)
        else:
...
def resnet34_peft(lora_r, adapter_r):
    ...
    model = ResNet(BasicBlockPEFT, [3, 4, 6, 3], num_classes=1000)
    model.load_state_dict(weights.get_state_dict(progress=True), strict=False)
    return model

def resnet50_peft(lora_r, adapter_r):
    weights = ResNet50_Weights.DEFAULT
    model = ResNet(BottleneckPEFT, [3, 4, 6, 3], num_classes=1000)
    model.load_state_dict(weights.get_state_dict(progress=True), strict=False)
    return model